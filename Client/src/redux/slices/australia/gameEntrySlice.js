...
export const getMyGameEntries = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      const url = queryString ? `/australia/game-entry?${queryString}` : "/" + COUNTRY + "/game-entry";
...
export const getUsercredit = createAsyncThunk(
  "australiaGameEntry/getcredit",
  async (_, { rejectWithValue }) => {
    try {
      const { data } = await api.get("/australia/game-entry/credit/me");
      return data.credit;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Something went wrong."
...
export const selectcredit = (state) => state.australiaGameEntries.credit;
